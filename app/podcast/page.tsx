...
const getWhatsAppUrl = (packageName: string, price: string, serviceType: string = "Podcast Setup") => {
...
export default function PodcastPage() {
  return (
    <div className="min-h-svh bg-[#0b0f19]">
      ...
      <section className="relative py-12 sm:py-16 md:py-20 px-4 sm:px-6 text-center min-h-svh flex items-center">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 px-4">
            <Link href="#packages">
              <button className="bg-gradient-to-r from-[#00b4d8] to-[#9d4edd] text-white px-6 sm:px-8 py-3 sm:py-4 rounded-lg shadow-lg hover:opacity-90 transition flex items-center justify-center gap-2 text-sm sm:text-base w-full sm:w-auto">
                View Packages <ArrowRight size={18} />
              </button>
            </Link>
            
            <Link href="/#contact">
              <button className="border border-[#00b4d8] text-[#00b4d8] px-6 sm:px-8 py-3 sm:py-4 rounded-lg hover:bg-[#00b4d8] hover:text-white transition text-sm sm:text-base w-full sm:w-auto">
                Get Custom Quote
              </button>
            </Link>
          </div>
        ...
        <div className="absolute bottom-4 sm:bottom-8 right-4 sm:right-8 z-20">
          <div className="flex items-center gap-2 bg-black/40 backdrop-blur-sm px-2 sm:px-3 py-1.5 sm:py-2 rounded-full text-white text-xs sm:text-sm">
            ...
            <span className="text-[10px] sm:text-xs">Live Background</span>
          </div>
        </div>
      </section>
          ...
          {packages.map((pkg, index) => (
            <motion.div
              ...
              className={`relative bg-gradient-to-br from-[#1a1f2e] to-[#2a2f3e] rounded-2xl p-6 sm:p-8 shadow-2xl hover:shadow-3xl transition-all duration-300 ${
                pkg.popular ? 'ring-2 ring-[#00b4d8] sm:scale-105' : ''
              }`}
            >
              {pkg.popular && (
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-baseline gap-2">
                  {pkg.customQuote ? (
                    <div className="flex flex-col">
                      <span className="text-2xl sm:text-3xl font-bold text-[#9d4edd]">{pkg.price}</span>
                      <span className="text-xs sm:text-sm text-gray-400 mt-1">{pkg.duration}</span>
                    </div>
                  ) : (
                    <>
                      <span className="text-3xl sm:text-4xl font-bold text-[#00b4d8]">{pkg.price}</span>
                ...
                <ul className="space-y-2 sm:space-y-3">
                  {pkg.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-start gap-2 sm:gap-3">
                      <Check className="w-4 h-4 sm:w-5 sm:h-5 text-[#00b4d8] mt-0.5 flex-shrink-0" />
                ...
                {pkg.customQuote ? (
                  <a 
                    href={getWhatsAppUrl(pkg.name, pkg.price, "Podcast Setup")}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block w-full"
                  >
                    <button className="w-full py-3 sm:py-4 rounded-lg font-semibold transition-all duration-300 border border-[#00b4d8] text-[#00b4d8] hover:bg-[#00b4d8] hover:text-white text-sm sm:text-base">
      ...
      {/* CTA Section */}
      <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 text-center bg-[#0b0f19]">
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
